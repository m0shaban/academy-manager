# 📚 دليل الـ API - نظام إدارة الأكاديمية v4.1

## 🔐 نظام الأمان والمصادقة

### 🔑 كود المدير الثلاثي
لحماية قصوى، يستخدم النظام كود مدير من 3 خطوات:
- **الخطوة 1**: بلح
- **الخطوة 2**: طرح  
- **الخطوة 3**: موز

## 🎫 API توليد الأكواد

### `POST /gen-vouchers`
**الوصف**: توليد أكواد اشتراك جديدة

**المصادقة**: كود المدير الثلاثي

**الطلب**:
```json
{
  "step1": "بلح",
  "step2": "طرح",
  "step3": "موز",
  "count": 20,
  "duration_days": 30
}
```

**الاستجابة الناجحة** (200):
```json
{
  "status": "ok",
  "count": 20,
  "duration_days": 30,
  "codes": [
    "ABC123XYZ",
    "DEF456UVW",
    "GHI789RST"
  ]
}
```

**الاستجابة عند الخطأ** (401):
```json
{
  "status": "error",
  "message": "كود المدير غير صحيح - تأكد من الخطوات الثلاث"
}
```

## 🔐 API تفعيل الاشتراك

### `POST /activate`
**الوصف**: تفعيل اشتراك مستخدم باستخدام كود

**المصادقة**: بدون (عام للمستخدمين)

**الطلب**:
```json
{
  "user_id": "facebook_user_12345",
  "code": "ABC123XYZ"
}
```

**الاستجابة الناجحة** (200):
```json
{
  "status": "activated",
  "subscription_end": "2026-02-12T10:30:00.000Z"
}
```

**الاستجابة عند الخطأ** (400):
```json
{
  "status": "error",
  "message": "الكود غير صحيح أو مستخدم بالفعل"
}
```

## 📊 API فحص حالة الاشتراك

### `GET /subscription-status?user_id=USER_ID`
**الوصف**: فحص حالة اشتراك المستخدم

**المعاملات**:
- `user_id` (مطلوب): معرف المستخدم
- `secret` (اختياري): مفتاح سري للوصول الإداري

**الاستجابة النشطة** (200):
```json
{
  "status": "active",
  "active": true,
  "subscription_end": "2026-02-12T10:30:00.000Z"
}
```

**الاستجابة المنتهية الصلاحية** (200):
```json
{
  "status": "expired",
  "active": false,
  "subscription_end": "2025-12-01T10:30:00.000Z"
}
```

## 🔄 APIs الإدارة والتحديث

### `POST /update-config?secret=SECRET_KEY`
**الوصف**: تحديث إعدادات البوت

**المصادقة**: مفتاح سري

**الطلب**:
```json
{
  "active_hours": [9, 11, 14, 17, 20, 22],
  "mood": "حماسي جداً",
  "rss_feeds": ["https://example.com/feed1", "https://example.com/feed2"]
}
```

### `GET /status`
**الوصف**: فحص حالة النظام العامة

**الاستجابة**:
```json
{
  "status": "running",
  "time_cairo": "2026-01-12 15:30:00",
  "active_hours": [9, 11, 14, 17, 20, 22],
  "rss_count": 30,
  "mood": "حماسي جداً",
  "last_post_hour": 14
}
```

## 💾 قاعدة البيانات

### جدول المستخدمين (`users`)
```sql
CREATE TABLE users (
    user_id TEXT PRIMARY KEY,
    subscription_end TEXT,
    created_at TEXT
);
```

### جدول الأكواد (`vouchers`)
```sql
CREATE TABLE vouchers (
    code TEXT PRIMARY KEY,
    duration_days INTEGER NOT NULL,
    is_used INTEGER DEFAULT 0,
    used_by TEXT,
    created_at TEXT,
    used_at TEXT
);
```

## 🛡️ دوال الحماية

### `is_premium(user_id: str) -> bool`
فحص ما إذا كان المستخدم لديه اشتراك نشط

### `activate_voucher(user_id: str, code: str) -> (bool, str)`
تفعيل كود اشتراك للمستخدم

### `generate_vouchers(count: int, duration_days: int) -> List[str]`
توليد مجموعة من أكواد الاشتراك

## 🔧 أمثلة التطبيق

### Python (Requests)
```python
import requests

# توليد أكواد
response = requests.post(
    "http://localhost:5000/gen-vouchers",
    json={
        "step1": "بلح",
        "step2": "طرح",
        "step3": "موز", 
        "count": 10,
        "duration_days": 30
    }
)

# تفعيل اشتراك
response = requests.post(
    "http://localhost:5000/activate",
    json={
        "user_id": "user123",
        "code": "ABC123XYZ"
    }
)

# فحص الحالة
response = requests.get(
    "http://localhost:5000/subscription-status?user_id=user123"
)
```

### JavaScript (Fetch)
```javascript
// توليد أكواد
fetch('/gen-vouchers', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    step1: 'بلح',
    step2: 'طرح',
    step3: 'موز',
    count: 10,
    duration_days: 30
  })
})
.then(response => response.json())
.then(data => console.log(data));
```

## 📋 رموز الخطأ

| رمز | الوصف | الحل |
|-----|--------|-----|
| 401 | كود المدير غير صحيح | تأكد من الخطوات الثلاث |
| 400 | بيانات الطلب ناقصة | تأكد من إرسال كل المعاملات المطلوبة |
| 404 | المسار غير موجود | تأكد من URL الصحيح |
| 500 | خطأ في الخادم | تحقق من logs الخادم |

---

🔐 **أمان قصوى** | 📚 **وثائق شاملة** | 🚀 **جاهز للإنتاج**